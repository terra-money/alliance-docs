...
Since the Alliance module mints and burns native staking tokens when rebalancing voting power, the Total Supply API needs to be updated to return accurate results. The custom wrapper only affects the following APIs: `/cosmos/bank/v1beta1/supply` & `/cosmos/bank/v1beta1/supply/by_denom`.
