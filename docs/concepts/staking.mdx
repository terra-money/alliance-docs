...
To preserve the role of a native staking asset and to prevent the power dilution of a chain's native assets, each Alliance asset added to a chain is given a  `RewardWeight`. This parameter determines the amount of staking rewards a staked Alliance asset will generate in proportion to a natively staked coin. A chain’s native staking asset always maintains a reward weight of `1`. This amount is referred to as the "total reward power"  or "[total voting power](#governance)" of an asset. 
...
In addition to the `RewardWeight` parameter, new Alliances can also specify a `RewardDelayTime`. This parameter defines the amount of time users have to stake their new Alliance assets before the consensus power of the Alliance asset is applied to the chain. This delay prevents early stakers from gaining outsized rewards with a relatively small total stake of a new Alliance asset. The default setting for this parameter is one day.
...
Although the term "voting power" and "reward power" are used to describe the same concept, delegators cannot vote on governance proposals using staked Alliance assets. The native coins minted by the Alliance module that represent the consensus power of Alliance assets are owned solely by the Alliance module, and therefore cannot be used by delegators to specify votes. Only delegators who stake native assets directly to validators can participate in governance. 
</Admonition>
...
Rebalancing is calculated using a proportion of each Alliance asset’s [voting/reward power](#governance), specified by the asset’s `RewardWeight`. In consensus, one staked native coin is equal to one vote. Therefore, the total reward power of staked Alliance assets must be proportional to the number of native coins staked. If an Alliance asset has a reward weight of `1`, the total voting/reward power of the Alliance asset’s staked supply is always equal to the number of staked native coins, regardless of the Alliance asset’s supply. 

The following example illustrates the rebalancing of voting/reward power when the staked supply of the native token changes. This example shows staking pools as total amounts across all validators. Note that the reward power proportions set by the reward weight must always be constant with the supply of the native staked asset, regardless of the supply change of Alliance assets.

![](/img/alliance/rebalance-example.png)

### Calculating a rebalance

The following equations illustrate how [voting/reward power](#governance) is calculated during rebalances.
...
The following example shows the voting/reward power rebalancing of a staked Alliance asset (`AL1`) when a user redelegates from one validator to another. Note that the total supplies of staked assets remain constant.
