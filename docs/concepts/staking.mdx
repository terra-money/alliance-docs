...
The following illustrtion demonstrates the voting power of different assets relative to their reward weights. 
...
To maintain the voting power proportions between native and Alliance assets, the Alliance module updates its staking delegations through minting and burning native coins. This process is called rebalancing. The rebalancing of the staked supply of assets updates the voting power of validators proportional to the `RewardWeight` parameters.
...
To conserve computing resources, rebalancing only occurs when there is a change and at most once at the end of a block. If there is any change in delegations during a block, the Alliance module will update its staking proportions to ensure that validator delegations reflect the state of staked assets.
