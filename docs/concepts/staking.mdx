...
In addition to the `RewardWeight` parameter, new Alliances can also specify a `RewardDelayTime`. This parameter defines the amount of time users have to stake their new Alliance assets before the consensus power of the Alliance asset is applied to the chain. This delay prevents early stakers from gaining outsized rewards with a relatively small total stake of a new Alliance asset. The [default setting for this parameter is one week](../tech/parameters.mdx).
