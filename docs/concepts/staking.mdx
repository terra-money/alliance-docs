...
To preserve the role of a native staking asset and to prevent the power dilution of a chain's native assets, each Alliance asset added to a chain is given a  `RewardWeight`. This parameter determines how much voting power a new Alliance asset has in proportion to the natively staked token. A chain’s native staking asset always maintains a reward weight of `1`.
...
In addition to the `RewardWeight` parameter, new Alliances can also specify a `RewardDelayTime`. This parameter defines the amount of time users have to stake their new Alliance assets before the voting power of the new asset is applied to the chain. This delay prevents early stakers from gaining outsized voting power with a relatively small total stake of a new Alliance asset. The default setting for this parameter is one week.
...
Rebalancing is calculated using a proportion of each Alliance asset’s voting power, specified by the asset’s `RewardWeight`. In governance, one staked native coin is equal to one vote. Therefore, the total voting power of staked Alliance assets must be proportional to the number of native coins staked. If an Alliance asset has a reward weight of `1`, the total voting power of the Alliance asset’s staked supply is always equal to the number of staked native coins, regardless of the Alliance asset’s supply. 
