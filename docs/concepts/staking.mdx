...
To preserve the role of a native staking asset and to prevent the power dilution of a chain's native assets, each Alliance asset added to a chain is given a  `RewardWeight`. This parameter determines the amount of staking rewards a staked Alliance asset will generate in proportion to a natively staked coin. A chain’s native staking asset always maintains a reward weight of `1`. This amount is reffered to as the "total reward power"  or "[total voting power](#governance)" of an asset. 
...
The following equations illustrate how [voitng/reward power](#governance) is calculated during rebalances.
