...
Voting power is rebalanced at the end of a block whenever the block contains an event that triggers a voting power change.
...
Because redelegating a native coin does not change the supply of the native coin, and because the voting power of native coins is always equal with the staked amount, the redelegation of native assets does not trigger a rebalance of voting power.
...
8. For each Alliance asset:
    - Calculate the expected native coins (voting power) that the validator should have based on the asset's reward weight and the amount of the asset. This amount should be the rebalanced voting power, as it takes into account the changes made during the block.
...
The `RewardHistories` data structure is used to track the amount of rewards eligible for delegations and the amount of rewards claimed by delegators without having to iterate through all delegations every block. `RewardHistories` track the rewards that a unit of voting power is eligible for.
...
3. For each snapshot iterated by ascending blockheight:
    - Compare the delegator’s `RewardHistories` the snapshot’s records to determine how many rewards to claim.
